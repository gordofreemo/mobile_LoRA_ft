#!/usr/bin/env python3
"""
Generate 100 per-user OPPU training configs from the Round-6 template.

Reads `train/config/user_lora_lamp4_oppu_template.json` and substitutes
per-user placeholders for each fingerprint in `LaMP_4_top100_users.json`:

  <<CONDITION>>     -> user_lora_lamp4_<user>_oppu
  <<DATASET_PATH>>  -> data/lamp_user_train_LaMP_4_<user>_bm25k4.jsonl
  <<OUTPUT_DIR>>    -> train/checkpoints/user_lora_lamp4_<user>_oppu_seed0

Output: `train/config/user_lora_lamp4_oppu_<user>.json` (100 files,
gitignored — deterministically derivable from the template + the top-100
JSON).

Note the output_dir carries an explicit `_oppu_` tag (unlike Round 5's LaMP-3
configs, which didn't need one) to disambiguate from the pre-existing
R1/R2-B/R4 single-user LaMP-4 checkpoints (`user_lora_lamp4_u00000011_seed0`,
`user_lora_lamp4_u00000011_bm25k4_seed0`,
`user_lora_lamp4_u00000011_records_bm25k4_seed0`) — u00000011 lands at rank 1
in this K=100 pool.

Per the Round-6 plan
(experiments/2026-06-19-user-lora-round6-lamp4-multi-plan.md §Step 5,
naming chain).

Usage (CPU-only, <1s):
    python data/lamp_user_stats/round6_gen_configs.py
    python data/lamp_user_stats/round6_gen_configs.py --overwrite
"""
...
DEFAULT_TEMPLATE = CONFIG_DIR / "user_lora_lamp4_oppu_template.json"
DEFAULT_TOP_USERS = USER_STATS_DIR / "LaMP_4_top100_users.json"


def per_user_substitutions(fp: str) -> dict:
    return {
        "<<CONDITION>>": f"user_lora_lamp4_{fp}_oppu",
        "<<DATASET_PATH>>": f"data/lamp_user_train_LaMP_4_{fp}_bm25k4.jsonl",
        "<<OUTPUT_DIR>>": f"train/checkpoints/user_lora_lamp4_{fp}_oppu_seed0",
    }
...
def main():
    ...
    if not args.top_users.exists():
        sys.exit(f"ERROR: missing top-users JSON {args.top_users}. Run Step 2 first.")
    ...
    out_paths = {
        fp: CONFIG_DIR / f"user_lora_lamp4_oppu_{fp}.json"
        for fp in fps
    }
    ...
    for fp in fps:
        ...
        assert cfg["condition"] == f"user_lora_lamp4_{fp}_oppu", cfg["condition"]
        assert cfg["dataset_path"].endswith(f"{fp}_bm25k4.jsonl"), cfg["dataset_path"]
        assert cfg["output_dir"].endswith(f"{fp}_oppu_seed0"), cfg["output_dir"]
        ...
        written += 1
    print(f"[gen] wrote {written} configs to {CONFIG_DIR}/user_lora_lamp4_oppu_<user>.json",
          flush=True)
