#!/usr/bin/env python3
"""
Select the top-K LaMP-4 users by profile size for the Round 6 User-LoRA
multi-user replication of OPPU.

Per `experiments/2026-06-19-user-lora-round6-lamp4-multi-plan.md` Step 2:

  - Eligible filter: `n_test >= 1` AND `seen_by_a1_lamp == 0`
    (253 LaMP-4 users qualify; pool size verified at run time).
  - Rank: descending by profile size (read from the user's test record's
    `profile` field in `data/lamp_time/LaMP_4/test_questions.json`).
  - Output: `data/lamp_user_stats/LaMP_4_topK_users.json` (K=100 default).

The 95 MB `test_questions.json` is streamed via `stream_json_array` — kept
...
For multi-test-record users (LaMP-4 users have 1-25 test records, unlike
LaMP-3's near-uniform 1) the `profile_size` reported is the max over the
user's test records; the recorded `test_record_id` is the one that produced
that max. This avoids ambiguity downstream — Step 8 will eval each user on
the test record listed here (smoke); the full eval in Step 9 covers ALL of
a user's test records, not just this one.

Usage (CPU-only, ~tens of seconds):
    python data/select_top_users_lamp4.py
    python data/select_top_users_lamp4.py --k 100 --overwrite
...
DEFAULT_CSV = USER_STATS_DIR / "LaMP_4_users.csv"
DEFAULT_RECORDS_JSON = USER_STATS_DIR / "LaMP_4_user_records.json"
DEFAULT_TEST_QUESTIONS = TIME_SPLIT_DIR / "LaMP_4" / "test_questions.json"
...
def main():
    ...
    parser.add_argument("--out", type=Path, default=None,
                        help="output path (default: "
                             "data/lamp_user_stats/LaMP_4_top<k>_users.json)")
    ...
    out_path = args.out or USER_STATS_DIR / f"LaMP_4_top{args.k}_users.json"
    ...
    print(
        f"[run] select_top_users_lamp4 k={args.k} commit={commit_short} "
